...
public function allVendors(Request $request)
{
    ...
    public function addVendors(Request $request)
     public function addVendorsOld(Request $request)
    {
    ...
    public function allVendors(Request $request)
    {
        ...
        if ($permission_role->view == '2') {
            $vendor = InventoryVendor::where('created_by', Auth::user()->id)
                ->where('is_deleted', '0')
                ->latest()->get();
        } elseif ($permission_role->view == '3') {
            $employees = Employees::where('manager_id', Auth::user()->id)->pluck('id')->toArray();
            ...
            $vendor = InventoryVendor::where('is_deleted', '0')
                ->latest()->get();
        } else {
            $vendor = collect([]);
        }
        ...
        $vendorsWithActions = $vendor->map(function ($row) use ($id, $permission_role) {
            $created_by = $row->created_by;
            $manager = Employees::where('id', $created_by)->first();
            ...
            if ($permission_role->edit == '2' && $id == $created_by) {
                $actions["can_edit"] = true;
                $actions["edit_url"] = route('editvendor', $row->id);
            } elseif ($permission_role->edit == '3' && $id == $manager?->manager_id) {
                $actions["can_edit"] = true;
                $actions["edit_url"] = route('editvendor', $row->id);
            } elseif ($permission_role->edit == '4' && ($id == $manager?->manager_id || $id == $created_by)) {
                $actions["can_edit"] = true;
                $actions["edit_url"] = route('editvendor', $row->id);
            } elseif ($permission_role->edit == '5') {
                $actions["can_edit"] = true;
            ...
            if ($permission_role->delete == '2' && $id == $created_by) {
                $actions["can_delete"] = true;
                $actions["delete_url"] = route('deletevendor', $row->id);
            } elseif ($permission_role->delete == '3' && $id == $manager?->manager_id) {
                $actions["can_delete"] = true;
                $actions["delete_url"] = route('deletevendor', $row->id);
            } elseif ($permission_role->delete == '4' && ($id == $manager?->manager_id || $id == $created_by)) {
                $actions["can_delete"] = true;
                $actions["delete_url"] = route('deletevendor', $row->id);
            } elseif ($permission_role->delete == '5') {
                $actions["can_delete"] = true;
    ...
    /**
     * POST: Add vendor with all your existing validations and conditions
     */
    public function addVendorsPost(Request $request)
    {
        ...
        if ($sub != '') {
            ...
            $vendor->gst_no = $request->gst_no;
            $vendor->zip = $request->zip;
            $vendor->address = $request->address ?? ''; // changed from $permanent_address
            $vendor->country_id = $request->country;
            $vendor->state_id = $request->state;
            $vendor->city_id = $request->city;
            if ($vendor->save()) {
